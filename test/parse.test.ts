...
describe("parse", async () => {
  ...
  it("should parse complex list operations", () => {
    ...
    const result = parse(input);
    expect(result).toEqual(["map", ["lambda", ["x"], ["*", "x", 2]], ["list", 1, 2, 3]]);
  });
  ...
  it("should parse complex nested expressions", () => {
    const input = "{... $1 notes (filter (lambda (note) (> note 60)) (notes $1)) highest (apply max (notes $1))}";
    const result = parse(input);
    expect(result).toEqual({
      type: "object",
      spread: "$1",
      properties: {
        notes: ["filter", ["lambda", ["note"], [">", "note", 60]], ["notes", "$1"]],
  ...
  it("should parse mixed list and object expressions", () => {
    const input = "(let ((factor (* 0.01 (tempo $1)))) {... $1 velocity (* (velocity $1) factor)})";
    const result = parse(input);
    expect(result).toEqual([
      "let",
      [["factor", ["*", 0.01, ["tempo", "$1"]]]],
